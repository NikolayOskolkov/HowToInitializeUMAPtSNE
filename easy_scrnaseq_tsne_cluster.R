#FUNCTION ACCEPTS RAW SCRNASEQ EXPRESSION MATRIX AND RETURNS TSNE PLOT WITH HDBSCAN CLUSTERING RESULTS
...
easy_scrnaseq_tsne_cluster<-function(df)
{
  ...
  perp_range[1]<-3
  if(N_cells/3 <= 500){perp_range[9] <- N_cells/3-1}else{perp_range[9] <- 500}
